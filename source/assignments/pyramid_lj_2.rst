...
* Create a list of learning journal entries in ``views.py``, containing today's entry, yesterday's entry, and at least the three days prior to that.
  Each entry should have attributes of "id", "title", "creation_date", and "body".
...
* Disable form submissions on the ``create`` and ``update`` views.
